...
<script>function greet(){const h=new Date().getHours();return h>=5&&h<12?'Dobro jutro':h>=12&&h<18?'Dobar dan':'Dobra večer'}function say(t){if(window.LanaNative&&LanaNative.isReady()){document.getElementById('status').textContent='Lana priprema glas…';LanaNative.speak(t)}else document.getElementById('status').textContent='Hrvatski glas još nije spreman.'}window.nativeVoiceReady=function(v){document.getElementById('status').textContent='✓ Lana glas spreman: '+v}window.nativeVoiceSpeaking=function(){document.getElementById('status').textContent='Lana govori…'}window.nativeVoiceDone=function(){document.getElementById('status').textContent='✓ Poruka izgovorena do kraja'}window.nativeVoiceError=function(e){document.getElementById('status').textContent='✕ Glas nije dostupan: '+e}</script></body></html>
""".trimIndent()
        ...
        if (ttsReady) {
            ...
            engine.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
                ...
                override fun onDone(utteranceId: String?) {
                    if (utteranceId?.startsWith("lana-file-") != true) return
                    val file = pendingAudioFile ?: return
                    runOnUiThread { playRenderedFile(file) }
                }

                @Deprecated("Deprecated in Java")
                override fun onError(utteranceId: String?) {
                    notifyWeb("nativeVoiceError", "TTS_FILE_ERROR")
                }

                override fun onError(utteranceId: String?, errorCode: Int) {
                    notifyWeb("nativeVoiceError", "TTS_FILE_ERROR_$errorCode")
                }
            })
    ...
    private fun playRenderedFile(file: File) {
        mediaPlayer?.stop()
        mediaPlayer?.release()
        mediaPlayer = MediaPlayer().apply {
            ...
            setOnErrorListener { mp, _, _ ->
                notifyWeb("nativeVoiceError", "AUDIO_PLAYBACK_ERROR")
                mp.release()
    ...
    inner class LanaVoiceBridge {
        ...
        fun speak(text: String): Boolean {
            ...
            runOnUiThread {
                ...
                val outFile = File(cacheDir, "lana-$session.wav")
                pendingAudioFile = outFile
                val utteranceId = "lana-file-$session"
                val result = engine.synthesizeToFile(text.trim(), null, outFile, utteranceId)
                if (result == TextToSpeech.ERROR) {
                    pendingAudioFile = null
                    outFile.delete()
                    notifyWeb("nativeVoiceError", "TTS_FILE_START_ERROR")
                }
            }
            return true
        }
